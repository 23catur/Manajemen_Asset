...
public class Rig_input extends AppCompatActivity {
    ...
    void validasiData(){
        if (!hostname.isEmpty() && !merk.isEmpty() && !unit.isEmpty() && !serialnumber.isEmpty() && !tanggal.isEmpty() && !keterangan.isEmpty()){
            kirimdata();
        }else{
            Hostname.setError("Masukkan Hostname!");
            Merk.setError("Masukkan Type / Merk!");
            Unit.setError("Masukkan Unit!");
